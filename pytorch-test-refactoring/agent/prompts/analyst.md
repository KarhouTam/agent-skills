...
4. **Classify every test** into one of three strategies:
   - **Strategy 1 (Accelerator-unrelated)**: No device usage, CPU-only. Keep original class name. When multiple S1 tests exist inside an otherwise-S2 class, recommend extracting them into a new CPU-only class.
