...
def commit_changes(file_path: str, branch: str) -> str:
    ...
    _run_git(
        ...
        f"Apply test decoupling strategies (CPU-only/device-agnostic/device-specific) to {file_path}.\n"
...
def create_draft_pr(branch: str, fork_remote: str, file_path: str) -> tuple[int, str]:
    ...
    body = (
        ...
        f"- **CPU-only**: tests with no device dependency (CPU only)\n"
        f"- **device-agnostic**: tests that run on any accelerator (device-parametrized)\n"
        f"- **device-specific**: tests for a single accelerator (CUDA/MPS/XPU)\n\n"
